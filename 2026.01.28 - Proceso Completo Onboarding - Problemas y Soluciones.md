...
**Estado Final**: Infraestructura completa desplegada con imagen personalizada del init-container. Pendiente de ejecutar corrección de URLs en BD y prueba final de onboarding.
...
4. Realm seeding de Keycloak usa imagen init-container - necesita personalización para URLs correctas
5. La configuración correcta es `realmSeeding.initContainer.image.name`, no `initContainer.image`
...
- ✅ CentralIDP (Keycloak)
- ✅ SharedIDP (Keycloak)
- ✅ BPDM (Gate, Pool, Orchestrator, Cleaning Service)
- ✅ PostgreSQL databases
- ❌ Portal (frontend/backend) - NO desplegado
- ❌ SSI components - NO desplegados
- ❌ Discovery services - NO desplegados
...
## 📝 Fase 1: Diagnóstico y Bypass de Clearinghouse (Revisión 13-14)

### 1.1. Análisis del Estado de TestCompany1
...
2. Navegar a Applications → TestCompany1
...
**Resultado**: TestCompany1 avanzó pero quedó en **PENDING** (no ACTIVE).

---

## 📝 Fase 2: Identificación de Componentes SSI Faltantes (Revisión 14-15)

### 2.1. Análisis de Process Steps Fallidos
...
**Componentes críticos para onboarding**:
1. **ssi-credential-issuer**: Emisión de Verifiable Credentials (OBLIGATORIO)
...
6. **bpndiscovery**: Búsqueda de EDC por type numbers (RECOMENDADO para data exchange)
7. **discoveryfinder**: Búsqueda de discovery endpoints (RECOMENDADO para data exchange)

### 2.3. Habilitación de Componentes en Values
...
**Nota sobre smtp4dev**: Es **OBLIGATORIO en entornos dev/test**. 
Durante el proceso de onboarding se envían **múltiples emails críticos** (típicamente 3 emails por business partner):
1. **Email con credenciales iniciales**: Password para primer acceso al portal (Subject: Invitation to register for Catena-X)
2. **Email de confirmación de registro**: Verificación de datos enviados (Subject: Password required to register for Catena-X)
3. **Email de aprobación**: Notificación cuando la aplicación es aprobada (Subject: Information: Catena-X Registration successfully submitted)

Además de un email al final del proceso de onboarding:

4. **Email de bienvenida a Catena-X**: Notificación de que el proceso de onboarding se ha completado (Subject: Welcome to the Catena-X Network.)

Sin smtp4dev no podrás capturar estos emails y por tanto no podrás:
- Acceder al portal con el password inicial
- Confirmar que el registro fue procesado
- Verificar el estado final de aprobación

Los emails capturados se visualizan en http://smtp4dev.51.68.114.44.nip.io. En producción se reemplazaría por un servidor SMTP real configurado que envíe a destinatarios reales.

---

## 📝 Fase 3: Despliegue SSI Infrastructure (Revisión 15-17)

### 3.1. Configuración de URLs en values-ovh-hosts-portal.yaml
...
**Resultado**: Revisión 15 desplegada, componentes SSI activos.

---

## 📝 Fase 4: Despliegue Discovery Services - Problema Ingress (Revisión 16-18)

### 4.1. Configuración Inicial de Discovery Services
...
bpndiscovery:
  bpndiscovery:
    ...
    ingress:
      enabled: true  # INICIAL - causó error
      className: "nginx"
    idp:
...
- Pero los subcharts hardcodean `pathType: Prefix` sin exponerlo como parámetro configurable

### 4.3. Solución: Deshabilitar Ingress de Subcharts + Crear Manual
...
**Resultado**: Revisión 16-17 desplegada exitosamente.

---

## 📝 Fase 5: Problemas PostgreSQL Image y URL Scheme (Revisión 17-18)
...
### 5.2. Solución: Cambiar a bitnamilegacy

**Actualización en values-adopter-portal-for-onboarding.yaml**:
...
**Solución en values-ovh-hosts-portal.yaml**:
...
**Resultado**: Revisión 18 desplegada.

### 5.5. Verificación de Discovery Services
...
✅ **Discovery services completamente operacionales**.

---

## 📝 Fase 6: Descubrimiento de Portal No Desplegado
...
### 6.2. Verificación de Bases de Datos
...
**Hallazgo Crítico**: NO existe `portaldb`. Esto significa que:
- Portal backend NO se ha desplegado nunca
- Portal frontend NO está desplegado
- No hay datos de TestCompany1 (se perdieron)

### 6.3. Verificación de Pods de Portal

```bash
kubectl get pods -n portal | grep -E "portal-frontend|portal-backend" | grep -v postgres
```

**Resultado**: `No resources found` (ningún pod de portal corriendo)

### 6.4. Verificación de Values
...
**Conclusión**: El portal nunca se ha desplegado. Para onboarding completo se necesita:
...
## 📝 Fase 7: Problema "Invalid parameter: redirect_uri" (Revisión 19-24)
...
### 7.2. Diagnóstico - Script de Verificación

**Creación de `check-keycloak-urls.sh`**:
...
**Hallazgo Crítico**: ❌ **Todas las URLs usan `.tx.test`** en lugar de `.51.68.114.44.nip.io`

### 7.3. Análisis Causa Raíz - Init-Container

**Verificación de imagen usada**:
```bash
...
1. Init-container contiene archivos JSON con configuración de Keycloak
...
```
CX-Central-realm.json          ← Contiene URLs .tx.test
...
## 📝 Fase 8: Creación de Imagen Personalizada del Init-Container (Revisión 20-24)

### 8.1. Actualización de Archivos realm.json

**Script de actualización** (`init-container/update-realm-urls.sh`):
...
**Resultado**:
```
📝 Procesando: CX-Central-realm.json
   ✅ Cambios realizados: 11 líneas modificadas
📝 Procesando: CX-Central-realm_MAssembly.json
   ✅ Cambios realizados: 11 líneas modificadas
✅ Actualización completada
```

### 8.2. Instalación y Configuración de Docker
...
### 8.3. Construcción de Imagen Personalizada
...
# Login
sudo docker login
# (Autenticación web-based completada)
...
✅ **Imagen disponible en**: `docker.io/xmendialdua/catena-x-init:v1`

### 8.5. Configuración en Values - INTENTO 1 (Fallido)
...
**Resultado**: Revisión 20 desplegada
...
❌ **Problema**: Sigue usando imagen oficial, no la personalizada.

### 8.6. Búsqueda de Configuración Correcta
...
**Búsqueda en GitHub**:
```bash
# Encontrado en documentación del chart centralidp:
# La estructura correcta es:
# realmSeeding.initContainer.image.name
```

### 8.7. Configuración Correcta - INTENTO 2 (Exitoso)

**Actualización de `values-ovh-hosts-portal.yaml`**:
...
**Resultado**: Revisión 21 desplegada
...
✅ **¡Éxito!** Ahora usa la imagen personalizada.
...
**Resultado**:
```
http://portal.tx.test/*  ← Todavía con .tx.test
```

**Causa**: Keycloak realm seeding NO sobrescribe `redirect_uris` si el cliente ya existe. La base de datos ya tenía datos anteriores.

**Solución Requerida**:
1. **Opción A**: Reiniciar bases de datos de Keycloak desde cero
2. **Opción B**: Ejecutar job de corrección SQL una última vez

---

## 📝 Fase 9: Reinicio de Bases de Datos Keycloak (Revisión 21-24)
...
# (PostgreSQL usa volúmenes efímeros, se borran automáticamente)
...
**Resultado**: Revisión 21 desplegada, PostgreSQL recreado con BD vacía.

### 9.2. Múltiples Intentos de Configuración

**Revisión 22-23**: Experimentos con diferentes configuraciones de `realmSeeding`.

**Revisión 24**: Configuración final correcta con imagen personalizada.
...
# Resultado: docker.io/xmendialdua/catena-x-init:v1 ✅
...
**Resultado**:
```
http://portal.tx.test/*  ← AÚN con .tx.test
```

**Causa**: A pesar de la imagen personalizada correcta, Keycloak NO actualizó `redirect_uris` porque:
1. Los clientes ya existían en la BD anterior
...
## 📝 Solución Pendiente: Job de Corrección SQL
...
Actualizado con configuración de imagen personalizada:
...
## 📊 Resumen de Revisiones Helm

| Revisión | Cambios Principales | Estado |
|----------|-------------------|--------|
| 13 | Estado inicial | Base |
| 14 | Bypass Clearinghouse (manual) | Parcial |
| 15 | Habilitación SSI components | Activo |
...
| 20 | Primera configuración init-container (incorrecta) | Imagen no usada |
| 21 | Reinicio BD Keycloak | BD limpia |
| 22-23 | Experimentos configuración | Iteraciones |
| 24 | Configuración realmSeeding correcta | **Imagen personalizada OK, URLs pendientes corrección** |
...
|------------|--------|-----|-------|
| CentralIDP | ✅ Running | http://centralidp.51.68.114.44.nip.io | Con imagen personalizada |
| SharedIDP | ✅ Running | http://sharedidp.51.68.114.44.nip.io | Con imagen personalizada |
| BPDM Gate | ✅ Running | http://business-partners.51.68.114.44.nip.io/gate | Requiere fix-bpdm-hosts.sh |
| BPDM Pool | ✅ Running | http://business-partners.51.68.114.44.nip.io/pool | Requiere fix-bpdm-hosts.sh |
| BPDM Orchestrator | ✅ Running | http://business-partners.51.68.114.44.nip.io/orchestrator | - |
| BPDM Cleaning Service | ✅ Running | - | Internal only |
| Self-Description Factory | ✅ Running | http://sdfactory.51.68.114.44.nip.io | - |
| SSI Credential Issuer | ✅ Running | http://ssi-credential-issuer.51.68.114.44.nip.io | - |
| SSI DIM Wallet Stub | ✅ Running | http://ssi-dim-wallet-stub.51.68.114.44.nip.io | - |
| BDRS Server | ✅ Running | http://bdrs-server.51.68.114.44.nip.io | - |
| BPN Discovery | ✅ Running | http://semantics.51.68.114.44.nip.io/bpndiscovery | Ingress manual |
| Discovery Finder | ✅ Running | http://semantics.51.68.114.44.nip.io/discoveryfinder | Ingress manual |
| SMTP4Dev | ✅ Running | http://smtp4dev.51.68.114.44.nip.io | - |
| PgAdmin4 | ✅ Running | http://pgadmin4.51.68.114.44.nip.io | - |

### Componentes NO Desplegados ❌

| Componente | Estado | Razón |
|------------|--------|-------|
| Portal Frontend | ❌ Not Running | `portal.enabled: false` en values |
| Portal Backend | ❌ Not Running | `portal.enabled: false` en values |
| Portal PostgreSQL | ❌ Not Running | `portal.enabled: false` en values |

---

## 📋 Checklist Proceso Completo para Mañana
...
### Paso 1: Preparación de Init-Container (Una sola vez)
...
### Paso 3: Configuración de Values
...
### Paso 4: Instalación Inicial

```bash
cd charts/umbrella

# Instalar (no upgrade, instalación desde cero)
...
**Resultado Esperado**: Revisión 1 desplegada.

### Paso 5: Esperar a Inicialización
...
```bash
# 1. Fix BPDM hosts (requerido después de cada upgrade)
cd charts/umbrella
./fix-bpdm-hosts.sh
...
**Si aparecen URLs con `.tx.test`**: Ejecutar job de corrección una sola vez:
```bash
kubectl apply -f fix-keycloak-urls-job-complete.yaml -n portal
...
### Paso 7: Verificación de Componentes
...
**Resultado Esperado**: Todos devuelven respuestas válidas.
...
**Proceso Onboarding**:
1. Registrar nueva compañía como TestCompany1
2. Rellenar formulario de registro
3. Submit application
4. **Automático**: Proceso en background ejecuta:
   - BPDM crea BPN
...
**Síntoma**: Después de helm upgrade, BPDM ingress vuelve a usar `.tx.test`

**Causa**: Subcharts de BPDM hardcodean hosts en templates

**Solución**: Ejecutar `fix-bpdm-hosts.sh` después de cada upgrade
...
**Solución**: Usar `bitnamilegacy/postgresql:15-debian-11`
...
**Solución**: Asegurar todas las URLs incluyen esquema

```yaml
discoveryfinderClient:
  baseUrl: "http://semantics.51.68.114.44.nip.io/discoveryfinder"  # ✅
  # NO: "semantics.51.68.114.44.nip.io/discoveryfinder"  # ❌
```

### Problema 5: Keycloak Redirect URI Invalid

**Síntoma**: "Invalid parameter: redirect_uri" al acceder al portal

**Causa**: URLs en realm.json con `.tx.test`, no coinciden con dominio real

**Solución Permanente**: Usar imagen personalizada del init-container

**Solución Temporal**: Ejecutar job de corrección SQL
...
**Síntoma**: Después de upgrade con imagen personalizada, URLs siguen con `.tx.test`

**Causa**: Keycloak realm seeding es idempotente, NO sobrescribe `redirect_uris` si cliente existe

**Solución**: 
- **Para instalación desde cero**: Imagen personalizada funciona correctamente
- **Para actualización**: Ejecutar job de corrección SQL una vez
...
| `fix-keycloak-urls-job-complete.yaml` | `charts/umbrella/` | Job SQL corrección URLs |

### Documentación

| Archivo | Ubicación | Propósito |
|---------|-----------|-----------|
| `README-realm-urls.md` | `init-container/` | Explicación problema URLs |
| `EXPLICACION-IMAGEN-PERSONALIZADA.md` | `init-container/` | Explicación detallada init-container |
...
|---------|-------------------|
| `values-adopter-portal-for-onboarding.yaml` | - Habilitados todos componentes FASE 1-4<br>- PostgreSQL image override para discovery<br>- Portal enabled: false → true (pendiente) |
| `values-ovh-hosts-portal.yaml` | - Agregadas URLs SSI components<br>- Agregadas URLs discovery services<br>- Configuración realmSeeding init-container<br>- Ingress deshabilitado discovery (manual) |
| `values-ovh-hosts-portal-template.yaml` | - Agregada configuración realmSeeding |

---

## 🎓 Lecciones Aprendidas

### 1. Arquitectura de Inicialización Keycloak

**Aprendido**:
- Init-container provee archivos JSON estáticos
...
**Implicación**: Para cambiar dominio, necesitas imagen personalizada O corrección SQL.

### 2. Estructura de Subcharts

**Aprendido**:
- Subcharts pueden tener valores hardcodeados
- No todos los parámetros son configurables desde umbrella
- Algunos requieren workarounds (scripts post-upgrade, ingress manual)

**Implicación**: Revisar templates de subcharts antes de asumir configurabilidad.

### 3. PostgreSQL Image Lifecycle

**Aprendido**:
- Bitnami depreca imágenes antiguas sin aviso
- `bitnamilegacy` registry contiene versiones deprecadas
- Importante usar misma versión en todos los componentes

**Implicación**: Estandarizar versión PostgreSQL en todos los subcharts.
...
- Nginx ingress controller valida combinación path + pathType

**Implicación**: Para rutas complejas, usar ingress manual si subchart no permite configurar pathType.

### 5. Service-to-Service Communication

**Aprendido**:
- URLs internas deben incluir esquema (http:// o https://)
- Java cliente HTTP estricto con validación URI
- Error críptico si falta esquema

**Implicación**: Siempre incluir `http://` o `https://` en todas las URLs de configuración.
...
**Implicación**: No se puede hacer onboarding parcial. Todos los componentes deben estar operacionales.
...
- ✅ Wildcard DNS (nip.io) funcionando
- ✅ Helm chart versión 3.14.5 desplegado

### Backend Services ✅

- ✅ CentralIDP con imagen personalizada
- ✅ SharedIDP con imagen personalizada
- ✅ BPDM completo (Gate, Pool, Orchestrator, Cleaning)
- ✅ SSI infrastructure completa
- ✅ Discovery services operacionales

### Pendiente ⏳

- ⏳ Verificación/corrección URLs Keycloak (ejecutar job SQL)
- ⏳ Portal frontend/backend deployment (cambiar enabled: true)
- ⏳ Prueba onboarding end-to-end
- ⏳ Validación credenciales emitidas
- ⏳ Prueba discovery EDC

---

## 🚀 Próximos Pasos Mañana

### Opción A: Continuar con Instalación Actual
...
### Opción B: Instalación Limpia (RECOMENDADO)

1. `helm uninstall portal -n portal`
2. Seguir checklist completo de este documento
3. Instalación desde cero con todos los componentes
4. Portal incluido desde el inicio
5. Imagen personalizada init-container desde inicio

**Recomendación**: **Opción B** para evitar problemas acumulados y tener instalación limpia con configuración final correcta desde el inicio.
...
- **Keycloak Realm Seeding**: Documentación en chart de centralidp
...
Este documento recoge **toda la información crítica** del proceso seguido hoy. Para mañana:

1. **Leer el Checklist completo** antes de empezar
2. **Opción B recomendada**: Instalación limpia
...
- ✅ Company status cambia a ACTIVE
- ✅ BPN asignado correctamente
- ✅ Credenciales emitidas y almacenadas
- ✅ Visible en Partner Network
- ✅ EDC discoverable vía discovery services

---

**Documento generado**: 28 Enero 2026  
**Última revisión Helm**: 24  
**Estado**: Infraestructura completa, pendiente validación onboarding
...
# Verificar configuración ingress
